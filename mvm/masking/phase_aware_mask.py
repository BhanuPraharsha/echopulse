...
class PhaseAwareMask:
    ...
    def __init__(self, mask_ratio=0.50, systole_frac=0.70, phase_labels=None):
        ...
        self.systole_frac = (
            systole_frac  # fraction of the masked frames that are systolic
        )
        self.phase_labels = (
            phase_labels  # per-frame labels: 0-diastole, 1=systole(if known)
        )
    ...
    def __call__(self, volume, phase_labels=None):
        ...
        sel_sys = (
            np.random.choice(sys_idx, n_sys, replace=False)
            if n_sys > 0
            else np.array([])
        )
        sel_dias = (
            np.random.choice(dias_idx, n_dias, replace=False)
            if n_dias > 0
            else np.array([])
        )
