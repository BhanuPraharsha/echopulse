...
class MultiTaskDecoder(nn.Module):
    ...
    def __init__(self, embed_dim=768, n_patches=49,
                 patch_pixels=256, n_phases=2):
        super().__init__()
        ...
        self.phase_head = nn.Sequential(# phase classification head
            ...
            nn.GELU(),
            nn.Linear(128, n_phases)# input: mean of all patch tokens for each frame, output: gives probability of each phase
        )
    ...
    def forward(self, encoder_out, T):
        ...
        recon = self.recon_head(
            rearrange(tokens, 'b t p e -> (b t) p e'))
