import torch, wandb, yaml, argparse
...
def train_epoch(enc, dec, masker, loss_fn, loader, opt, dev, ep):
    ...
    for vid, ef, _ in tqdm(loader, desc=f'Epoch {ep}'):
        ...
        for i in range(B):
            mv, m = masker(v2d[i])
            mvids.append(mv); masks.append(m)
...
def validate_epoch(enc, dec, masker, loss_fn, loader, dev, ep): # same as train_epoch but no updating of weights
    enc.eval(); dec.eval(); total = 0
...
def main(cfg):
    ...
    train_ds = EchoNetDataset('data/echonet/splits/train.csv',
                              max_frames=max_frames,
                              augment=True) 
    val_ds   = EchoNetDataset('data/echonet/splits/val.csv',
                              max_frames=max_frames) 
    ...
    # best = float('inf') #to add this
    # for ep in range(1, cfg['epochs'] + 1): #to add this
