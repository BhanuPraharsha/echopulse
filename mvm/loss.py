...
@dataclass
class LossOut:
    total: float; recon: float; phase: float; ef: float
...
class MVMLoss(nn.Module):
    ...
    def forward(self, recon_pred, recon_true, mask,
                phase_pred, phase_true, ef_pred, ef_true):
        ...
        return LossOut(L_total.item(), L_recon.item(),
                       L_phase.item(), L_ef.item()), L_total